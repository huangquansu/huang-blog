...
class BlogPost(db.Model):
    ...
    # need to be given a post,then foreignkey can work(child)
...
class Comment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    author_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    # need to be given a user,then foreignkey can work(child)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to login or register to commenr")
            return redirect(url_for('login'))
        new_comment = Comment(
            text=form.body.data,
    ...
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated, user_id=current_user.get_id(), form=form, comments=comment)
